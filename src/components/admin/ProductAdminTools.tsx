...
export default function ProductAdminTools() {
  ...
  async function importCsv(file: File) { const form = new FormData(); form.append("file", file); const response = await fetch("/api/commerce/admin/imports/products", { method: "POST", body: form }); show(response.ok ? { variant: "success", title: "Importação concluída", message: "Arquivo processado com sucesso." } : { variant: "error", title: "Falha na importação", message: "Não foi possível processar o arquivo." }); if (response.ok) router.refresh(); }
