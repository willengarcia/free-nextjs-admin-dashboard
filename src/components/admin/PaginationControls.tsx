...
export default function PaginationControls({ page, totalPages, total }: { page: number; totalPages: number; total: number }) {
  ...
  function go(next: number) { const params = new URLSearchParams(searchParams.toString()); params.set("page", String(next)); params.set("size", "20"); router.push(`${pathname}?${params}`); }
